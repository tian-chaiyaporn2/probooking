# Internal tools (Operations / Finance / Admin)
...
> [`apps/web/src/app/ops`](../apps/web/src/app/ops) and
> [`apps/web/src/app/finance`](../apps/web/src/app/finance) — role-guarded dashboards that
> call the controlled `/ops/*` and `/finance/*` API actions (verify, hold/resolve,
> reconciliation, export, metrics, audit). There is no separate `apps/ops` package.

...
| BKG-01..04 | Atomic confirmation, immutable snapshots, derived status | `domain: eligibility, state-machines/booking`, `api: BookingsService`, `db: Booking`; BKG-03 `termsSnapshot` now freezes the real agreed terms (it was persisted as `{}`) and the booking carries the offer's snapshot forward | `05-*`, `14-*` | 🟡 (BKG-02 atomicity rests on the transaction + unique constraints but has **no test** that a mid-confirm failure leaves no orphan records) |
...
| §6.4 | Integrity: one booking/shift, no dup payout, different-person approval, immutable audit | `db: unique constraints (Booking.shiftId/offerId, FinancialEvent.idempotencyKey, `Offer_one_active_per_shift` partial unique); state preconditions asserted **inside** the payout/cancel/confirm transactions; `AuditRecord` written for confirm/payout/cancel + every ops action | `05-*`, `11-*`, `14-*` | 🟡 (**dual control NOT implemented**: `requiresDualControl`/`dualControlSatisfied` exist in `domain/roles` but no endpoint takes a second approver — `19-*` covers the predicate only. Audit rows are app-append-only, not DB-enforced) |

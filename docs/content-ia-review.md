...
> **Implementation status (2026-07-18):** the P0–P2 recommendations below were implemented on `master` via the UX review pass (#20 — role-aware nav, Thai statuses, candidate names, checkout/cancel modals, arrive→complete gate, verification review endpoints, richer finance/ops copy) and the offer-decline follow-up. This review is retained as the rationale behind those changes; the original `cursor/information-architecture-flows-d877` branch is superseded by #20.

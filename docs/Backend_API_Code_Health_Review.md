...
- **H4 (confirm-transaction TOCTOU): done.** `confirmBooking` now re-checks §6.3 eligibility (credential kind, insurance, suspension, expiry) and AVL-03 overlap **inside** the transaction under a professional row lock (`SELECT … FOR UPDATE`), so a suspend/lapse/expiry or concurrent confirm between the controller's pre-capture read and the write cannot create an illegal booking. A new in-transaction `acceptOffer` claims `PendingResponse → AwaitingPayment` under the same lock, closing the concurrent-accept race; the in-memory store mirrors it with a synchronous check-then-claim (no `await` gap). `PaymentFailed` offers now count as soft holds, and `suspendCredential` / `listActiveBookings` target the profession-required credential kind instead of `credentials[0]`. Also closed the confirm-unwind money gap (part of H5): a failed refund after a failed confirm now surfaces (500 + order ref) instead of silently stranding captured funds. Covered by `marketplace-bugfixes.test.ts`, `prisma-store.int.test.ts` (row-lock concurrency), and store-parity. (#74)

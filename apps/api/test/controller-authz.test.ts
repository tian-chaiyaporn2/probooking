...
import { ReportingController } from "../src/modules/marketplace/controllers/reporting.controller.js";
...
function makeController(resolved: CallerIdentity) {
  ...
  // The reporting endpoints touch only the access service (authz) and the repo (the read).
  const access = new MarketplaceAccessService(repo);
  const controller = new ReportingController(access, repo);
...
describe("MarketplaceController reporting authz (REP-01)", () => {
  describe("professionals/:id/bookings", () => {
    for (const role of ["operations", "finance", "administrator"]) {
      it(`lets ${role} read any professional's history WITHOUT resolving them to a party`, async () => {
        const { controller, resolveIdentity, listPartyBookings } =
          makeController(identity({}));
        await expect(
          controller.professionalBookings("pro-1", token(role)),
        ).resolves.toEqual({
          bookings: [],
        });
    ...
    it("lets a professional read their OWN history", async () => {
      const { controller, resolveIdentity } = makeController(
        identity({ professionalId: "pro-1" }),
      );
      await expect(
    ...
    it("rejects an unauthenticated caller", async () => {
      ...
      await expect(
        controller.professionalBookings("pro-1", undefined),
  ...
  describe("clinics/:id/bookings", () => {
    it("lets finance read any clinic's history without resolving them to a party", async () => {
      ...
      await expect(
        controller.clinicBookings("clinic-1", token("finance")),
      ).resolves.toEqual({
        bookings: [],
      });
      expect(resolveIdentity).not.toHaveBeenCalled();
    ...
    it("lets a clinic member read their OWN workspace's history", async () => {
      const { controller } = makeController(
        identity({
          memberships: [
            { workspaceId: "clinic-1", role: "clinic_admin" as never },
          ],
        }),
      );
      await expect(
        controller.clinicBookings("clinic-1", token("clinic_admin")),
      ).resolves.toEqual({
        bookings: [],
      });
    });
    ...
    it("forbids a non-member reading a clinic's history", async () => {
      const { controller } = makeController(
        identity({
          memberships: [
            { workspaceId: "other", role: "clinic_admin" as never },
          ],
        }),
      );
      await expect(

...
| VER-01..07 | Verification states, Ops verify, gating, holds | `domain: advanceVerification (VER-02)`, `api: /ops/*/verify` + `/suspend-credential`, confirm gates on verification (§6.3), VER-06 hold overlay (`Booking.heldAt`) via `worker: credentialHoldSweep` → `/bookings/:id/hold-credential` (blocks payout + auto-accept, opens case, notifies), `/resolve-hold` | `01-*`, `13-*` | 🟡 (01–04/06/07 live) |
